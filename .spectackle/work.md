...
## P-01KYDBRWFZFXSBRF4PNRD6R4D9 the git workflow becomes server automation: state transitions drive branch, push, draft pull request, ready and merge
kind: proposal
state: active
created: 2026-07-25
refs: ADR-01KYDBQGMRFBN9SAHCWWNSAKX4, T-01KYDA92TGF3FT994TEE6EDVN6
grilled: 2026-07-25
targets: internal/mcpserver, internal/wt, internal/workspace

Requirement: no change may exist unpushed or uncovered; a task in progress is its own branch under a DRAFT pull request; a finished task's pull request is ready for review; and the server, not the LLM, performs every one of those steps mechanically from the state transition that implies it.

WHAT ALREADY EXISTS, and why this is not a duplicate. T-01KYDA9 (submitted) states this policy in prose and is explicitly documentation-only, no Go code; its own rejected predecessor was superseded for a different reason (it armed auto-merge, which the redraft removed). The policy therefore exists as text nobody enforces. This proposal is the enforcement, and the two must not disagree: T-01KYDA9's CONTRIBUTING section has to be reconciled with ADR-01KYDB rather than left contradicting it, since CONTRIBUTING.md as merged today still mandates auto-merge with squash.

BINDING PRECEDENT from the rejection corpus, which constrains the design more than anything else here. The edge-commit engine draft was rejected once for colliding with B-0006: wt.CommitCode deliberately excludes .spectackle, and preserveSpectackle exists because concurrent journal writes were a real, observed defect. Record state reaches the default branch through the semantic replay (SPX-SWM-001), never through a git merge of journal files. So this automation commits and pushes CODE, and must never start git-merging .spectackle content between branches. Any design that does is already known to be wrong here.

DECISIONS TAKEN, recorded rather than rediscovered.
- Merge authority: ADR-01KYDB. The server merges after task verification, always a merge commit, never squash; under insufficient repository permission it stops after opening the pull request, and says so.
- Two modes, online is the default. Online talks to the forge over HTTPS for pull-request create, ready and merge. Offline does the entire lifecycle in the local repository and merges into the local default branch, so the workflow is identical without a network and the test suite needs no forge stub.
- Opt-out, not opt-in: enabled wherever it can work, disabled by config. This changes behavior for existing users on upgrade, which is the accepted cost of the invariant being real rather than advisory.
- No gh dependency. Branch, commit, push and merge are plain git, which internal/wt already shells out to. Pull-request create, ready and merge need the forge API and are done over HTTPS directly; credentials resolve from the environment first, then from git's own credential helper, so nothing new has to be configured by a user who can already push.

TRANSITION MAPPING, the automation's whole surface:
  work op=start, or a move into active: ensure the task branch, commit, push, open a DRAFT pull request naming the task ID.
  any server write while the task is active: commit and push, so no change is ever only local.
  move to done: flip the pull request to ready for review.
  archive after verification: merge, merge-commit, never squash.

REJECTED ALTERNATIVES. Shelling out to gh: the requirement excludes it, and it puts a second auth system and a binary dependency between the server and an API it can call directly. Making the coupling opt-in: an invariant that holds only where someone remembered to enable it is not an invariant. Blocking every transition on the network: state moves would stop working offline and every lifecycle test would need a forge, which is why the offline mode exists instead.

EXIT CRITERION: on a repository with the feature enabled, driving a task from active to archived produces a branch, a draft pull request, a ready pull request and a merge commit without a single git or gh command issued by the LLM; the same sequence offline produces the same branch and a local merge into the default branch; disabling it in config restores today's behavior exactly; and .spectackle content is never git-merged between branches.
...
targets: internal/forge

IMPLEMENTER IN OWN WORKTREE. New package, no existing file changes at all — this is the most isolated half of P-01KYDB and can run concurrently with every sibling.

WHAT TO BUILD: internal/forge, one interface and two implementations.

The interface is the four operations the transition mapping needs and nothing else: open a draft pull request for a branch, mark a pull request ready for review, merge it, and look up the pull request already open for a branch (so the automation is idempotent — a second call must find the existing one, never open a duplicate).

Implementation one, GitHub over HTTPS. net/http only; no gh binary and no new module dependency (the requirement excludes gh, and a second auth system plus a binary dependency is the reason). Endpoints are the REST pulls API: create with draft true, patch to clear draft, and the merge endpoint with merge_method=merge — NEVER squash, per ADR-01KYDB and the sibling never-squash policy. Resolve the owner and repository from the git remote URL, accepting both https and ssh remote forms.

CREDENTIALS, in this order, first hit wins: GITHUB_TOKEN then GH_TOKEN from the environment; then git's own credential helper, `git credential fill` fed protocol/host on stdin, reading back the password field. The helper path is what makes this work with zero new configuration for anyone who can already push, and it is why gh is unnecessary rather than merely excluded. A token that resolves but lacks permission must be reported as exactly that — see the degradation rule below.

Implementation two, offline. Every operation succeeds locally and records what it would have done; merge performs a real local merge into the repository's default branch (merge commit, never squash, never fast-forward-only). This exists so the whole lifecycle runs with no network and so the test suite needs no forge stub — do not implement it as a no-op that silently does nothing.

DEGRADATION, not failure: insufficient permission on merge leaves the pull request open and returns a distinguishable result the caller can report, rather than an error that looks like a bug. ADR-01KYDB requires the automation to be safe to run as an identity that cannot merge, and requires the difference to be visible rather than silent.

TESTS: an httptest server standing in for the API covering create-draft, ready, merge, the already-open lookup, and the 403-lacks-permission path; remote URL parsing for https and ssh forms including a trailing .git; credential resolution preferring the environment and falling back to the helper, with the helper stubbed through an injected runner (do not shell out to a real git in a unit test); and the offline implementation performing a genuine local merge, asserted by reading the resulting git history rather than by trusting a return value.

VERIFY (real output, never predicted): go build ./... ; go test ./internal/forge/... -race ; go test ./... ; go vet ./... ; gofmt -l . (empty).
SCOPE, disjoint by construction: only the new internal/forge directory. Touch NOTHING else — not internal/mcpserver, not internal/wt, not internal/workspace, and no file under any .spectackle directory (server-owned records; siblings run concurrently).
ROLLBACK: delete the package; nothing imports it until the wiring task lands, so removal is total and affects no existing behavior.
REPORT BACK: the interface, how credentials resolved and how you tested each path without a network, how the offline merge is verified, each verification command's real result, anything deliberately not done.
...
parent: P-01KYDBRWFZFXSBRF4PNRD6R4D9
refs: ADR-01KYDBQGMRFBN9SAHCWWNSAKX4
targets: internal/workspace/workspace.go, internal/wt/wt.go

IMPLEMENTER IN OWN WORKTREE. Runs concurrently with the internal/forge task; the file sets do not overlap.

PART ONE, the config block. workspace.Config gains a git section, defaulted so the feature is OPT-OUT: present and enabled wherever it can work, switched off by one key. Fields: enabled (default true), mode (online default, offline the alternative), remote (default origin), base (default the repository's own default branch, not a hardcoded main — read it, do not assume), and the merge method fixed to a merge commit. Defaults are applied in workspace.load beside every other default already there, and an unknown mode is a config error at load, matching how ignore_regex is validated today rather than failing later at the point of use.

Why defaults matter here more than usual: this is an opt-out feature on a released tool, so the zero value of a config that predates it must mean enabled-and-online, and a workspace with no git remote at all must degrade quietly rather than erroring on every call.

PART TWO, the git primitives. internal/wt already shells out to git (see its git helper and CommitCode) and already knows about worktrees; extend it with exactly what the transition mapping needs and no more: ensure a branch exists and is checked out for a task, commit the CODE changes, push the branch to the configured remote setting upstream on first push, and report whether the branch has unpushed commits.

THE ONE CONSTRAINT THAT OVERRIDES CONVENIENCE, from B-0006 and SPX-SWM-001, and the reason an earlier version of this idea was rejected: CommitCode EXCLUDES .spectackle deliberately, and preserveSpectackle exists because concurrent journal writes were a real observed defect. Record state reaches the default branch through the semantic replay, never through a git merge of journal files. Your primitives commit and push CODE. Do not add .spectackle content to any commit these functions make, and do not introduce any path that git-merges .spectackle between branches. If you believe the task cannot be done without it, stop and report rather than doing it.

TESTS: defaults applied to a config that omits the block entirely (enabled, online, origin); an explicit disable honored; an unknown mode rejected at load with a message naming the key; the default branch read from the repository rather than assumed; branch-ensure idempotent (second call on an existing branch is a checkout, not an error); push setting upstream on first push, asserted against a real local bare remote rather than a mock; the unpushed-commits report true before push and false after; and a commit made by these primitives containing NO .spectackle path, asserted by reading the commit's file list.

VERIFY (real output, never predicted): go build ./... ; go test ./internal/workspace/... ./internal/wt/... -race ; go test ./... ; go vet ./... ; gofmt -l . (empty) ; spectackle lint . (POSITIONAL path).
SCOPE, disjoint: internal/workspace/workspace.go and internal/wt plus their tests. Do NOT touch internal/forge (a sibling is creating it), internal/mcpserver, internal/index, or any file under a .spectackle directory.
ROLLBACK: the config block is additive and defaults to today's behavior when disabled; the primitives are new functions nothing calls until the wiring task lands.
REPORT BACK: the config shape and its defaults, how you read the default branch, the proof that no primitive commits .spectackle, each verification command's real result, anything deliberately not done.
