...
PRIOR ART, checked before designing; both surfaced by this record's own rejections pack.

T-01KYDJ8A3YEQ7RSQ0QM74G69SN proposed a background CI poller pushing red verdicts onto the next tool result, and was rejected: the user refined the model to a BLOCKING await, where the LLM waits on the transition call while the server polls, so the verdict lands in that same call's result. This record does NOT reopen that. The blocking-await model is kept exactly as chosen; the only change is declining to spend the budget on a wait whose outcome is already determined before it starts. The verdict still returns in the same call - immediately and honestly, instead of after 12 minutes of guaranteed nothing. Nothing moves to the background.
