...
VERIFY: go build ./... && go test ./... -count=1 && gofmt -l . empty at EVERY phase commit; final: the offline lifecycle commit log pasted, the per-test migration outcome table pasted, full online suite green (byte-identity control group: worktree_e2e/invariants no-remote online fixtures). SCOPE: gitflow.go, tools.go comments, swarm.go wording, forge/offline.go header, internal/wt/wt.go (HeadSHA helper only), workspace config parsing + struct comment, bench.go+agent.go, the named tests + control-group fixture pins, docs. NON-GOALS: forge interface unchanged; swarm flow unchanged; no new config keys beyond honoring mode: online. ROLLBACK: revert the PR. REPORT: anything the single-branch model makes impossible - say it, do not approximate.
