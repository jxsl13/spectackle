...
// BuildDupIndex fingerprints spans, honors the token floor, excludes
// generated files, and reports truncation honestly.
func TestBuildDupIndex(t *testing.T) {
	g := graph.NewMem()
	g.Upsert([]graph.Node{
		{ID: "go:a.Big", Kind: graph.KFunc, File: "a/x.go", Line: 1, EndLine: 14},
		{ID: "go:a.Tiny", Kind: graph.KFunc, File: "a/x.go", Line: 16, EndLine: 17},
		{ID: "go:gen.Skipped", Kind: graph.KFunc, File: "gen/z.go", Line: 1, EndLine: 14},
	}, nil)
	files := map[string]string{
		"a/x.go":   gateExcerpt + "\n\nfunc tiny() {}\nvar _ = tiny\n",
		"gen/z.go": "// Code generated by tool. DO NOT EDIT.\n" + gateExcerpt,
	}
	idx, truncated := BuildDupIndex(g, func(p string) []byte { return []byte(files[p]) })
	if truncated {
		t.Fatal("three nodes reported truncated")
	}
	ids := map[string]bool{}
	for _, n := range idx {
	...
	if !ids["go:a.Big"] {
		t.Fatalf("real function missing from the index: %v", ids)
	}
	if ids["go:a.Tiny"] {
		t.Fatal("sub-floor span indexed")
	}
	if ids["go:gen.Skipped"] {
		t.Fatal("generated file indexed")
	}
}
