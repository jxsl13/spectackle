...
// braceSpan finds the end of the body opened on lines[start] (a Def hit's
// line): if that line has no '{' at all, the def has no body — a prototype
// ending in ';', a function-like macro, ... — and ok is false, so forward
// lines (which may belong to the next, unrelated def) are never touched. If
// the def line does open a brace, depth-counts forward ('{' +1, '}' -1)
// until depth returns to <= 0, single-line bodies included (open and close
// on the def's own line).
func braceSpan(lines []string, start int) (end int, ok bool) {
	depth, opened := braceDelta(lines[start])
	if !opened {
		return 0, false
	}
	if depth <= 0 {
