...
// .claude/commands/spectacle.md pioneered — without hand-maintaining N
...
// commandsData is the template data — {Binary, Tool} are both "spectacle"
...
const generatedHeader = "<!-- generated by spectacle `commands` — edit internal/mcpserver/templates, not this file -->"
...
const (
	sectionBegin = "<!-- spectacle:commands:begin -->"
	sectionEnd   = "<!-- spectacle:commands:end -->"
)
...
func (s *Server) commandsGen(ctx context.Context, req *mcp.CallToolRequest, in commandsIn) (*mcp.CallToolResult, any, error) {
	...
	// in realtime, but no journal write — these files aren't .spectacle state.
...
func (s *Server) commandsElicit(ctx context.Context, req *mcp.CallToolRequest) ([]string, bool) {
	...
	res, err := req.Session.Elicit(ctx, &mcp.ElicitParams{
		Message: "spectacle: " + commandsQuestion,
...
func renderCommandTemplate(name string) (string, error) {
	...
	data := commandsData{Binary: "spectacle", Tool: "spectacle"}
...
// writeClaudeDialect regenerates .claude/commands/spectacle.md and
// spectacle-state.md — same frontmatter shape (description:) the
...
func writeClaudeDialect(root, workflowBody, stateBody string) ([]string, error) {
	...
	state := "---\ndescription: Render the current spectacle state (explicit alias for bare /spectacle)\n---\n\n" +
		generatedHeader + "\n\n" + stateBody
	wf := filepath.Join(dir, "spectacle.md")
	sf := filepath.Join(dir, "spectacle-state.md")
...
// writeCopilotDialect writes .github/prompts/spectacle.prompt.md and
// spectacle-state.prompt.md with GitHub Copilot's `mode: agent` frontmatter.
func writeCopilotDialect(root, workflowBody, stateBody string) ([]string, error) {
	...
	workflow := "---\nmode: agent\n---\n\n" + generatedHeader + "\n\n" + workflowBody
	state := "---\nmode: agent\n---\n\n" + generatedHeader + "\n\n" + stateBody
	wf := filepath.Join(dir, "spectacle.prompt.md")
	sf := filepath.Join(dir, "spectacle-state.prompt.md")
	if err := os.WriteFile(wf, []byte(workflow), 0o644); err != nil {
		return nil, err
	}
	if err := os.WriteFile(sf, []byte(state), 0o644); err != nil {
...
// writeAgentsSection creates or updates AGENTS.md's spectacle-managed
...
func agentsSection(workflowBody, stateBody string) string {
	...
	b.WriteString("## spectacle workflow\n\n")
