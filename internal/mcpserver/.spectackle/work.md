...
OBSERVED LIVE (2026-07-27, T-01KYHAH1GJ landing): the serving root is a linked worktree whose branch carries git: mode: online in .spectackle/config.yaml, but every gitflow edge reads s.main.Cfg.Git.Mode - the PRIMARY checkouts config, which predates the change - so the done edge silently ran the offline commit-only path (no ready flip, no CI) until the primary checkouts config file was hand-edited. The recon round had flagged this as one of two latent s.main/s.ws splits (the other: forgeFor rooted s.main while commits run s.ws). MECHANISM: config is loaded per root; a worktree-served repo has TWO .spectackle/config.yaml files that can disagree, and mode/enabled/remote/base all resolve from main while the edges operate on ws. EXPECTED: one documented resolution rule. Options to grill: (a) serving root wins for MODE (the tree the edges run on decides how they run; main keeps remote/base which are repo-global); (b) main wins but a divergence between the two configs renders a w-line on every flow result (never-silent); (c) load config from main only and document that worktree-local config.yaml is ignored. Option (a) matches the edges operating tree; option (b) is the minimal never-silent fix. TEST: fixture with a linked worktree serving root whose config sets mode online while mains says nothing (and the inverse) - assert the chosen rule and the divergence line. VERIFY: go build ./... && go test ./internal/mcpserver/ -count=1 && gofmt -l . empty. SCOPE: config resolution at the gitflow entry + docs. ROLLBACK: revert.
