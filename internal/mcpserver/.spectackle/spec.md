...
- T-0123 compact reports redundant rejection clusters; draft and rule describe the shared ID counter: compact dry-run reports redundant rejection clusters, reusing events the handler already read; draft and rule now state the shared coord.db counter. The load-bearing test hashes the raw journal before and after, and the implementer proved apply=true leaves it byte-identical too -- there is no supersede path. Archive was lost once to an orchestrator cwd error and redone here; the code itself was recovered by cherry-pick and verified green.
- P-0089 compact reports redundant rejection clusters; draft and rule stop implying IDs are predictable: delivered by T-0123.
- T-0126 generated pre-commit hook keeps records off worktree branches; make dev recovers a lost pidfile: Pre-commit hook installed via commands op=install-hooks, POSIX sh, refuses .spectackle paths staged from a linked worktree while letting code-only worktree commits and main-checkout commits through; verified live, mode 0755. make dev-stop now recovers a live listener with no pidfile. The implementer accidentally killed the orchestrator server on the default port while testing that branch and reported it unprompted -- the incident showed the kill rule was too broad, so the orchestrator added two guards: refuse a listener that is not a spectackle serve process, and refuse one whose working directory is a different workspace. Both proven live, the second by recovering the orchestrator own server. Only lsof and fuser exist here; the ss codepath is implemented but unverified.
- P-0092 generate a pre-commit hook that keeps record files off worktree branches, and harden make dev: delivered by T-0126 plus the ownership guards the test incident revealed.
