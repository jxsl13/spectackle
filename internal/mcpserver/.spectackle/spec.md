...
- T-0130 worktree submit end to end under the four conditions the old tests never had: one end-to-end submit violating all four old test-bed assumptions at once: primary branch not named main with a diverged stale main, primary advancing on the same journal paths while the worktree is open, live uncommitted bundle state in the worktree, and submit through a fresh server instance rooted at the worktree. Each of B-0002, B-0004, B-0005 and B-0006 reintroduced separately and shown to fail it, then restored. A second record-only submit covers the empty-commit case explicitly.
