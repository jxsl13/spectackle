...
`.claude/commands/spectacle.md`/`spectacle-state.md` carry — for every
...
output: **claude** → `.claude/commands/spectacle.md` +
`spectacle-state.md` (`description:` frontmatter, as today). **copilot** →
`.github/prompts/spectacle.prompt.md` + `spectacle-state.prompt.md`
(`mode: agent` frontmatter). **codex**/**kimi** → one managed section in
`AGENTS.md`, delimited by `<!-- spectacle:commands:begin -->`/`<!-- …:end
...
file. Every generated artifact carries a `<!-- generated by spectacle
...
repo files, not `.spectacle/` lifecycle state: no journal event, just one
