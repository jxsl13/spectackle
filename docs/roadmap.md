...
| **M4** *(perf gate cleared 7-15x; streamable-HTTP live; linter fuzzing live; cursors-everywhere still open)* | Hardening | cursors everywhere; linter fuzzing; perf: ≥100k LOC repo indexed < 5 s warm; optional streamable-HTTP transport | perf target met on a reference repo |
