...
| **M5** ✅ *(exit criterion met: features shipped through the server's own lifecycle all cycle)* | **Self-hosting gate** | CI: lint + check + coverage threshold; CONTRIBUTING documents the loop: every change starts with `find scope=rejection` + `draft` | a spectackle feature is merged that was developed through spectackle's own lifecycle |
| **M6** *(cookbook live + Fortran/ObjC/Metal parsers shipped; GLSL source parsed (30 languages); ObjC/gpupipe resolvers and wazero backend still open — Vulkan host-binding resolver remains module-provenance work, cgo stays past M6 per ADR-0011, wazero gated on wasi-sdk CUDA/ObjC grammar wasm per ADR-0010)* | Extension | ObjC/Metal/Vulkan resolvers; **wazero/WASM parser backend replaces cgo tree-sitter** (`CGO_ENABLED=0` release builds); new-language cookbook | adding a language touches only `langs.go` + one parser + one resolver |
